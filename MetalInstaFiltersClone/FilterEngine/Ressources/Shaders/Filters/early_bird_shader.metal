...
[[stitchable]] half4 early_kernel_main(
                                    ...
                                    )
{
    ...
    half3 bcs_result = Utils().brightnessContrastSaturation(irgb, 1.2, 1.1, 1.2);

    // More red, less blue
    half3 rb_result = half3(bcs_result.r * 1.1, bcs_result.g, bcs_result.b * 0.9);

    // Add filter (overlay blending)
    half3 after_filter = mix(rb_result, Utils().multiplyBlender(rb_result, filter), 0.8);
