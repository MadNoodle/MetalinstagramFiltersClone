...
[[stitchable]] half4 xpro_kernel_main(float2 position, half4 currentColor,
                                        texture2d<half> tex
                                        )
{
    ...
    half3 bcs_result = Utils().brightnessContrastSaturation(irgb, 1.2, 1.2, 1.3);

    // Add filter (overlay blending)
    half3 after_filter = mix(bcs_result, Utils().multiplyBlender(bcs_result, filter), 0.7);
